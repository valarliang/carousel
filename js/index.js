...
//开头动画
Array.from(img).forEach((e,i,all)=>{
    setTimeout(function () {
        move({obj:e,attrs:{top:0},d:3000,fx:'elasticOut'});
    },i*100+500);
    setTimeout(function () {
        under();
    },(all.length-1)*100+1300);
});
setTimeout(function () {
    toggleShape();
},200)
...
function under(){
    ...
    box.onmousemove = function(ev){
        ...
        box.style.left=(window.innerWidth-box.offsetWidth)/2+'px';
        let shape = document.getElementById('shape');
        removeClassName(shape,'shapeAim');
        let ratio=(ev.clientX-this.offsetLeft)/this.offsetWidth;
        shape.style.transform=`rotateY(-${ratio*360-16}deg)`;
        //循环图片
